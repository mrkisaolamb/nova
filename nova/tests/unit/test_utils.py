...
class SpawnOnTestCase(test.NoDBTestCase):
    ...
    def test_spawn_on_warns_on_full_executor(self, mock_warning):
        ...
        # Start two tasks that will wait
        utils.spawn_on(executor, task)
        utils.spawn_on(executor, task)

        # We expect that spawn_on will warn on the second task being submitted
        # as the executor is already full.
        mock_warning.assert_called_once_with(
            'The %s pool does not have free threads so the tasks %s is '
